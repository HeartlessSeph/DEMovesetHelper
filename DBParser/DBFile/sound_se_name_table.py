...
def prep_workspace(file_jsons):
    req_files = ["sound_se_name_table",
                 "sound_voicer",
                 "sound_category",
                 "sound_se_all_id_to_id"]
    if check_workspace(req_files, file_jsons): return
    sound_se_name = file_entry(file_jsons, "sound_se_name_table", False)
    ...
    for cat_num in [e for e in sound_se_name["subTable"] if e.isdigit()]:
        voicer_name = list(sound_se_name["subTable"][cat_num].keys())[0]
        for entry in [e for e in sound_se_name["subTable"][cat_num][voicer_name]["1"] if e.isdigit()]:
            sound_id = list(sound_se_name["subTable"][cat_num][voicer_name]["1"][entry].keys())[0]
            voice_table = str(sound_se_name["subTable"][cat_num][voicer_name]["1"][entry][sound_id]["2"])
            sound_eff_id = sound_se_name[voice_table][""]["se_name"]
            new_dict[voicer_name][sound_id] = sound_eff_id
    file_jsons["Final"]["sound_se_name_table"] = new_dict
    return
